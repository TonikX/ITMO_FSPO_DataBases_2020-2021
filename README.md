...
- Полученный SQL-код (*.sql) и отчет залить в папку этого репозитория **students/группа/laboratory_works/фамилия_имя/laboratory_work_5**.
- Дописать документацию из работы №4 и добавить в нее раздел "Запросы", в котором для каждого запроса описать:
    - Номер запроса
    - Описание запроса
    - Код запроса
